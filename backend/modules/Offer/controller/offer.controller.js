...
const createOffer = async (formData, userId, locale) => {
    ...
    let offer = new Offer({...formData, id, owner: userId, categoryIds: formData.categoryIds})
    await offer.save()
...
const uploadOfferImages = async (locale, userId, offerId, files, serviceName,  fileType) => {
    let offer=await Offer.get(offerId)
    console.log("offer是：",offer)
    if(!Offer){
        throw {
    ...
    for (const file of files) {
        ...
        urls.push(`http://${process.env.AWS_S3_BUCKET}.s3.localhost.localstack.cloud:4566/${key}`)
    }
...
module.exports = {createOffer, editOffer, getOffer, getOffers, deleteOffer, offerTemplate, uploadOfferImages}